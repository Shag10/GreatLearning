public class MedianOfTwoSortedArrayOfDifferentLength {
  ...
  public double findMedianSortedArrays(int input1[], int input2[]) {
    ...
    while (low <= high) {
      ...
      //if partitionX is 0, there is nothing on the left side. Use -INF for maxLeftX
      ...
      if (maxLeftX <= minRightY && maxLeftY <= minRightX) {
        ...
        // or get the max of left for odd length combined array size.
        if ((x + y) % 2 == 0) {
          return ((double) Math.max(maxLeftX, maxLeftY) + Math.min(minRightX, minRightY)) / 2;
        } else {
          return (double) Math.max(maxLeftX, maxLeftY);
        }
      } else if (maxLeftX > minRightY) { //we are too far on right side for partitionX. Go on the left side.
        high = partitionX - 1;
      } else { //we are too far on left side for partitionX. Go on the right side.
        low = partitionX + 1;
      }
    }
  ...
  public static void main(String[] args) {
    ...
    System.out.print(mm.findMedianSortedArrays(x, y));
  }
}
